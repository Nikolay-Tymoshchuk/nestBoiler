...
# Копируем скрипт startup.document.dev.sh в директорию /opt/
COPY ./startup.document.dev.sh /opt/startup.document.dev.sh

# Делаем скрипт startup.document.dev.sh исполняемым
...
# Удаляем символы возврата каретки из скрипта startup.document.dev.sh (для совместимости с Unix)
RUN sed -i 's/\r//g' /opt/startup.document.dev.sh

# Устанавливаем рабочую директорию в /usr/src/app
WORKDIR /usr/src/app

# Если файл .env не существует, копируем env-example-document в .env
